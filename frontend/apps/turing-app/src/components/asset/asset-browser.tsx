import { AssetPreviewPanel } from "@/app/console/asset/asset-preview-panel"
...
interface AssetBrowserProps {
  /** URL the LoadProvider "try again" link points at (usually the assets route). */
  tryAgainUrl: string
  /** Extra classes for the browser column (e.g. the console `px-4 lg:px-6`). */
  className?: string
}

/**
 * The asset file-browser body (MinIO/storage) — toolbar + breadcrumb + file
 * table + preview panel + RAG training — extracted so the console page and the
 * Bento asset surface (T561) share it without duplicating ~290 lines. Folder
 * navigation is component state (prefix), not URL routes, so it is route-shell
 * agnostic; the caller owns the surrounding header/padding chrome.
 */
export function AssetBrowser({ tryAgainUrl, className }: Readonly<AssetBrowserProps>) {
  ...
  return (
    <LoadProvider checkIsNotUndefined={assets} error={error} tryAgainUrl={tryAgainUrl}>
