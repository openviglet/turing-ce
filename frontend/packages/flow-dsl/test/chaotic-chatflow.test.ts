...
function nodeById(flow: TranspiledFlow, id: string) {
...
describe("flow-dsl new capabilities", () => {
  ...
  it("transpiles a humanApproval node with its nested config under data.humanApproval", () => {
    ...
    const config = (gate.data as Record<string, unknown>).humanApproval as Record<string, unknown>;
  ...
  it("copies outputVariable + continueOnFailure onto a functionCall node", () => {
    ...
    const fn = nodeById(flow, "fn").data as Record<string, unknown>;
  ...
  it("copies continueOnFailure onto a scheduleAgent node", () => {
    ...
    const sched = nodeById(flow, "sched").data as Record<string, unknown>;
  ...
  it("still rejects a non-blank switch sourceHandle that matches no option", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("opt-typo"),
    );
  });
  ...
  it("rejects more than one wildcard edge on a switch", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("wildcard"),
    );
  });
...
describe("transpileBundle", () => {
  it("validates cross-flow subFlowId references and fills subFlowName", () => {
    ...
    assert.equal((go?.data as Record<string, unknown>).subFlowName, "Child Flow");
  });
  ...
  it("throws on a dangling subFlowId reference", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("ghost"),
    );
  });
  ...
  it("throws when a bundled flow has no id", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("no id"),
    );
  });
  ...
  it("validates subFlowSwitch option subFlowId references too", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("nope"),
    );
  });
});
...
describe("chaotic-chatflow bundle (the giant stress fixture)", () => {
  ...
  it("declares one persona per (tone, style) register on the main flow", () => {
    ...
    assert.deepEqual(
      [...styles].sort(),
      ["DIRECT", "INSTRUCTIONAL", "NARRATIVE", "NEUTRAL", "PERSUASIVE"],
    );
  });
  ...
  it("wires functionCall + scheduleAgent failure edges and a switch wildcard", () => {
    const main = bundle[0];
    assert.ok(main);
    const fnFailure = main!.graph.edges.find(
      (e) => e.source === "fn-finalize" && e.sourceHandle === "failure",
    );
    assert.ok(fnFailure, "functionCall failure edge present");
    const schedTimeout = main!.graph.edges.find(
      (e) => e.source === "sched-finalize" && e.sourceHandle === "timeout",
    );
    assert.ok(schedTimeout, "scheduleAgent timeout edge present");
    const wildcard = main!.graph.edges.find(
      (e) => e.source === "switch-wild" && (e.sourceHandle === null || e.sourceHandle === ""),
    );
  ...
  it("covers every onJudgeReject policy across aiQuestion nodes", () => {
    ...
    for (const node of bundle[0]!.graph.nodes) {
      const v = (node.data as Record<string, unknown>).onJudgeReject;
      if (v !== undefined) policies.add(v);
    }
  ...
  it("populates subFlowName on cross-flow references", () => {
    ...
    assert.equal((subPaper?.data as Record<string, unknown>).subFlowName, "Stamp & Paperwork Office");
    const sandwich = bundle[1]!;
    const sfs = sandwich.graph.nodes.find((n) => n.id === "subflowswitch-abyss");
    const opts = (sfs?.data as Record<string, unknown>).switchOptions as Array<Record<string, unknown>>;
