...
describe("transpileFlow", () => {
  ...
  it("copies only whitelisted data fields per node type", () => {
    ...
    const askData = askNode.data as Record<string, unknown>;
  ...
  it("carries formCapture formFields through to the wire shape (T107)", () => {
    ...
    const formData = formNode.data as Record<string, unknown>;
    const fields = formData.formFields as Array<Record<string, unknown>>;
  ...
  it("validates condition branches", () => {
    assert.throws(
      ...
      (err: unknown) =>
        err instanceof FlowSpecError && err.message.includes("Condition node 'cond'"),
    );
  });
  ...
  it("validates switch sourceHandle ids", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("Switch node 'sw'"),
    );
  });
  ...
  it("rejects duplicate node ids", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("Duplicate node id"),
    );
  });
  ...
  it("rejects missing or duplicate start nodes", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("'start'"),
    );
    assert.throws(
      () => transpileFlow({
        name: "TwoStarts",
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("2 'start' nodes"),
    );
  });
  ...
  it("flags unreachable nodes (no incoming edges)", () => {
    assert.throws(
      ...
      (err: unknown) => err instanceof FlowSpecError && err.message.includes("orphan"),
    );
  });
  ...
  it("transpiles scheduleAgent nodes with routine fields and timeout edge", () => {
    ...
    const data = scheduleNode.data as Record<string, unknown>;
    ...
    const timeoutEdge = transpiled.graph.edges.find(
      (e) => e.source === "fire-proposal" && e.sourceHandle === "timeout",
    );
  ...
  it("transpiles subFlowSwitch nodes with per-option subFlow links", () => {
    ...
    const data = sfsNode.data as Record<string, unknown>;
    assert.equal(data.label, "SUB-FLOW SWITCH");
    assert.equal(data.switchVariable, "segment");
    const options = data.switchOptions as Array<Record<string, unknown>>;
