 ...
 * @since 2026.3.1
 */
export function transpileFlow(spec) {
 ...
 * @since 2026.3.1
 */
export function transpileBundle(specs) {
    ...
    for (const spec of specs) {
        ...
        if (!id) {
            throw new FlowSpecError(`Flow '${spec.name}' has no id — every flow in a bundle needs a stable id for cross-flow subFlowId wiring.`);
        }
        if (idToName.has(id)) {
    ...
    for (const spec of specs) {
        for (const ref of collectSubFlowRefs(spec)) {
            if (!idToName.has(ref.subFlowId)) {
                throw new FlowSpecError(`Flow '${spec.id}' node '${ref.nodeId}' references subFlowId '${ref.subFlowId}', ` +
...
function collectSubFlowRefs(spec) {
    const refs = [];
...
function fillSubFlowNames(transpiled, idToName) {
...
function validate(nodes, edges) {
    ...
    for (const node of nodes) {
        if (node.type !== "start" && !incomingTargets.has(node.id)) {
            throw new FlowSpecError(`Node '${node.id}' (${node.type}) has no incoming edge — it is unreachable.`);
        }
    }
}
function validateConditionBranches(nodeId, outgoing) {
    ...
    if (yesCount !== 1 || noCount !== 1 || outgoing.length !== 2) {
        throw new FlowSpecError(`Condition node '${nodeId}' must have exactly two outgoing edges with sourceHandle "yes" and "no" (got yes=${yesCount}, no=${noCount}, total=${outgoing.length}).`);
    }
}
function validateSwitchBranches(node, outgoing) {
    ...
    for (const edge of outgoing) {
        ...
        if (!optionIds.has(edge.sourceHandle)) {
            throw new FlowSpecError(`Switch node '${node.id}' outgoing edge to '${edge.target}' uses sourceHandle '${edge.sourceHandle}' — must match one of: ${[...optionIds].join(", ")} (or be left blank for the wildcard fallback).`);
        }
    }
    if (wildcards > 1) {
        throw new FlowSpecError(`Switch node '${node.id}' has ${wildcards} wildcard (blank-sourceHandle) edges — keep at most one as the no-match fallback.`);
    }
}
/* ─────────────────────── Per-node materialization ─────────────────────── */
function buildNode(node, index) {
    const label = node.label ?? DEFAULT_LABEL[node.type];
    const data = { label, type: node.type };
 ...
 */
function dedupeSlots(declared, nodes) {
...
function slotReferencedBy(node) {
    switch (node.type) {
        case "aiQuestion":
        case "formCapture":
