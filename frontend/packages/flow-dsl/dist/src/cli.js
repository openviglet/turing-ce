...
export async function build(inputDir, outputDir) {
    ...
    for (const entry of readdirSync(resolvedInput)) {
        ...
        try {
            ...
            failed++;
            const message = err instanceof FlowSpecError ? err.message : err.message;
            // eslint-disable-next-line no-console
...
function classify(filename) {
    if (filename.endsWith(".flow.json"))
        return "json";
    if (filename.endsWith(".flow.mjs") || filename.endsWith(".flow.js"))
        return "module";
 ...
 */
function baseName(filename, flowName) {
...
function looksLikeTranspiled(value) {
    if (typeof value !== "object" || value === null)
        return false;
    const obj = value;
 ...
 */
function reverseTranspile(transpiled) {
    const nodes = transpiled.graph.nodes.map((n) => {
        const { type: _ignoreInnerType, ...rest } = (n.data ?? {});
        return { id: n.id, type: n.type, ...rest };
    });
...
async function main() {
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`turing-flow-dsl: ${err.message}`);
        process.exitCode = 1;
    }
}
